...
use lcovutil qw (set_tool_name define_errors parse_ignore_errors
                 ...
                 info $verbose init_verbose_flag debug $debug
                 do_mangle_check
                 apply_rc_params
                 strip_directories
...
{
    my $self = shift;
    return unless @coverageCriteriaScript;
...
sub _computeAge
{
    ...
    my $now =
        ...
        DateTime->now();
    my $then = DateTime::Format::W3CDTF->parse_datetime($when);
    if ($then > $now) {
        my $data =
            exists($ENV{SOURCE_DATE_EPOCH}) ?
...
sub _load
{
    ...
    if (!-r $path) {
        if ($main::synthesizeMissingFile ||
            ...
            ($main::no_sourceview                         &&
             ...
             0 == scalar(@SourceFile::annotateScript)     &&
        ...
        die($self->path() .
                " is not readble or doesn't exist. See --syntheszie-missing option for workaround."
        );
    }

    # check for version mismatch...
    if (@lcovutil::extractVersionScript) {
        my $version = lcovutil::extractFileVersion($path);
    ...
    my $lineNum = 0;
    my @cmd     = @annotateScript;
    my $cmd     = join(' ', @cmd) . ' \'' . $repo_path . '\'';
...
sub new
{
    ...
    foreach my $f ($current_data->files()) {
        my $filename = ReadCurrentSource::resolve_path($f);
...
my (@rc_datebins, @rc_filter, @rc_ignore,
    ...
    @rc_omit_patterns, @rc_erase_patterns);
...
lcovutil::apply_rc_params(\@opt_config_file,
                          {
                           ...
                           "forget_testcase_names" =>
                               \$TraceFile::ignore_testcase_name,
                          });
...
if ($sort) {
    ...
    if ($show_functionProportions) {
        ...
        push(@funcview_sortlist, $SORT_MISSING_BRANCH) if ($br_coverage);
    }
}
...
if (@SummaryInfo::coverageCriteriaScript) {
...
Operation:
  ...
  -b, --baseline-file BASEFILE      Use BASEFILE as baseline file
      ...
      --show-zero-columns           Keep summary columns whose entires are all
                                    zero - default is to improve readability
                                    by removing these columns.
  ...
  -p, --prefix PREFIX               Remove PREFIX from all directory names
      ...
      --erase-functions PATTERN     Exclude coverpoints contained in matching
                                    function name
...
sub process_file($$$$$)
{
    ...
    $r .= " functions=$fn_found hit=$fn_hit"
        if $main::func_coverage && $fn_found != 0;
    $r .= " branches=$br_found hit=$br_hit"
        if $main::br_coverage && $br_found != 0;
    info($r . "\n");
...
sub write_function_table(*$$$$$$$$$$)
{
    ...
    if ($main::show_functionProportions) {
        ...
        $branchProportionRow = "<td class=\"tableHead\">$branch_code</td>"
            if $main::br_coverage && %$funcBranchCovMap;
    }
