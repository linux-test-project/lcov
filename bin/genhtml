...
sub simplify
{
    ...
    foreach
        my $bin (LINE_OWNERS, LINE_CATEGORIES, BRANCH_OWNERS, BRANCH_CATEGORIES)
    {
...
sub write_summary_pages($$$$$$$$)
{
    ...
    if ($main::show_tla) {
        ...
        foreach my $t ('line',
                       $main::br_coverage ? 'branch' : undef,
                       $main::func_coverage ? 'function' : undef) {
...
sub process_file($$$$$)
{
    ...
    my ($lines_found, $lines_hit, $br_found, $br_hit, $fn_found, $fn_hit) = (
                                       $fileData->found(), $fileData->hit(),
                                       $fileData->b_found(), $fileData->b_hit(),
                                       $fileData->f_found(), $fileData->f_hit()
    );
        ...
        if ($lcovutil::func_coverage) {
            ...
            if (%$differentialFunctionMap) {
                foreach (@funcview_sortlist) {
                    write_function_page([$differentialFunctionMap, \%lineCov,
                                         \%branchCov
                                        ],
                                        $base_dir,
                                        $rel_dir,
...
sub write_file_table(*$$$$$$)
{
    ...
    my @head_columns;
    my $includeFunctionColumns =
        $lcovutil::func_coverage && 'owner' ne $primary_key;
