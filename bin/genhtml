...
sub new
{
    ...
    $self->{line_owners} = {};  # developer -> hash of TLA->count - lineCov data
    $self->{branch_owners} = {};   # developer -> hash of TLA->count - branchCov
...
sub relativeDir
{
    ...
    die("bad usage")
        unless ((!defined($dir_string) && defined($self->{relativeDir})) ||
                (defined($dir_string) && !defined($self->{relativeDir})));
    $self->{relativeDir} = $dir_string
...
{
    my ($self, $owner) = @_;
    return exists($self->{line_owners}->{$owner});
}
...
sub owners
{
    ...
    my $hash = $self->{line_owners}
        if (!defined($covType) || $covType eq 'line');
    $hash = $self->{branch_owners}
...
sub owner_tlaCount
{
    ...
    die("$name not found in owner data for $self->path()")
        unless exists($self->{line_owners}->{$name});

    return 0    # not supported, yet
        if $covType eq 'function';

    my $ownerKey = $covType . "_owners";
...
sub findOwnerList
{
    ...
    my @owners;
    foreach my $owner (keys(%{$self->{line_owners}})) {
        my $lineMissed = $self->owner_tlaCount($owner, 'missed', 'line');
        my $branchMissed =
        ...
        if ($all ||
            ...
            my $lineTotal = $self->owner_tlaCount($owner, 'found', 'line');
            my $branchTotal =
                $lcovutil::br_coverage ?
...
sub checkCoverageCriteria
{
    ...
    if (grep(/^owner$/, @criteriaCallbackTypes)) {
        foreach my $t ('line', 'branch') {
            my $key = $t . '_owners';
            next unless exists($self->{$key});
            my $ownerBins = $self->{$key};
    ...
    if (defined($branch->[$loc])) {
        ...
        foreach my $branchId ($current->blocks()) {
            if (!$branchData->hasBlock($branchId)) {
                ...
                my $which = $loc == 0 ? 'baseline' : 'current';
                lcovutil::ignorable_error($lcovutil::ERROR_INCONSISTENT_DATA,
...
sub baseline_branch
{
    ...
    if (defined($branchData)) {
        $self->_mergeBranchData(0, $branchData, $filename);
    }
    my $branch = $self->[4];
    return $branch->[0];
}
...
sub current_branch
{
    ...
    if (defined($branchData)) {
        $self->_mergeBranchData(1, $branchData, $filename);
    }
    my $branch = $self->[4];
    return $branch->[1];
}
...
sub _categorizeBranchCov
{
    ...
    my $branchBaseline = $base_data->sumbr();
            ...
            if (defined($end_line)) {
                ...
                for (my $line = $curr->line(); $line <= $end_line; ++$line) {
                        ...
                        defined($lineCovCurrent->value($line)))   # line is code
                    {
                        ...
                        # line is same
                        my $bline = $linemap->lookup($filename, 'old', $line);
    ...
    while (<$diffHdl>) {
        ...
        foreach ($line) {
            ...
            /^--- (.+)$/ && do {
                if ($filename) {
                    my $key =
                        $lcovutil::case_insensitive ? lc($filename) : $filename;
                    push(@{$self->{linemap}->{$key}}, $chunk);
                    undef $filename;
                }
            ...
            /^\+\+\+ (.+)$/ && do {
                ...
                my $key =
                    $lcovutil::case_insensitive ? lc($file_new) : $file_new;
...
sub new
{
    ...
    $self->{_line_owners} = {};          # owner -> hash of TLA->list of lines
    $self->{_categories}  = {};          # TLA -> list of lines

    $self->{_branch_owners} = {};        # owner -> hash of TLA->list of lines
...
sub _countBranchTlaData
{
    ...
    foreach my $branchId ($differentialData->blocks()) {
        ...
        foreach my $b (@$diff) {
            ...
            my $ohash = $fileSummary->{branch_owners};
            ...
            if (!exists($recorded{$ownerKey})) {
                $recorded{$ownerKey} = 1;
                my $dhash = $self->{_branch_owners}->{$developer};
                $dhash->{$tla} = []
...
sub _countLineTlaData
{
    ...
    if ($SummaryInfo::tlaLocation{$tla} & 0x1) {
        ...
        if (defined($l->owner())) {
            ...
            # first:  increment line count in 'file summary'
            my $ohash = $fileSummary->{line_owners};
            $ohash->{$developer} = {}
            ...
            #   to this owner
            $self->{_line_owners}->{$developer} = {}
                unless exists($self->{_line_owners}->{$developer});
            my $dhash = $self->{_line_owners}->{$developer};
            $dhash->{lines} = []
...
sub isProjectFile
{
    ...
    my $self = shift;
    return scalar(%{$self->{_line_owners}});
}
...
sub nextInOwnerBin
{
    ...
    exists($self->{_line_owners}->{$owner}) &&
        exists($self->{_line_owners}->{$owner}->{$tla}) or
    ...
    my $locations = $self->{_line_owners}->{$owner}->{$tla};
    my $idx       = binarySearchLine($locations, $line);
    return defined($idx) ? $locations->[$idx] : undef;
}
...
sub nextBranchInOwnerBin
{
    ...
    if (exists($self->{_branch_owners}->{$owner})) {
        my $od = $self->{_branch_owners}->{$owner};
        if (exists($od->{$tla})) {
            my $l   = $od->{$tla};
...
sub _load
{
    ...
    if (!defined($repo_path)) {
        if (0 == scalar(@annotateScript) &&
            ...
            lcovutil::ignorable_error($ERROR_SOURCE,
        ...
        if (open(HANDLE, "-|", $cmd)) {
            while (my $line = <HANDLE>) {
                ...
                if ($commit ne 'NONE') {
                    ...
                    $self->{_line_owners}->{$owner} = {}
                        unless exists($self->{_line_owners}->{$owner});
                    my $dhash = $self->{_line_owners}->{$owner};
                    $dhash->{lines} = []
                        unless exists($dhash->{lines});
...
sub new
{
    ...
    foreach my $f ($current_data->files()) {
        ...
        if (!File::Spec->file_name_is_absolute($filename)) {
            ...
            $filename = File::Spec->catfile($parentDir, $filename);
        }
        my $short_name =
...
sub compute
{
    ...
    WORK: while (@$worklist ||
        ...
        if (1 < $lcovutil::maxParallelism) {
            ...
            # remove current object from dependencies...
            if ($type ne 'top') {
                my $mergeInto = $main::flat ? '' : $parentPath;
                $self->completed_dependency($mergeInto, $name);
            }
            my $end = Time::HiRes::gettimeofday();
...
my @datebins;
my (@rc_date_bins, @rc_annotate_script, @rc_criteria_script);
...
my %genhtml_options = (
              ...
              'source-directory=s' => \@ReadCurrentSource::source_directories,
...
@SourceFile::annotateScript = @rc_annotate_script
    unless @SourceFile::annotateScript;
@SummaryInfo::coverageCriteriaScript = @rc_criteria_script
    unless @SummaryInfo::coverageCriteriaScript;
...
{    # save copy of .info and diff files: useful for debugging user cases later
    foreach my $from ($base_filename, $diff_filename, @info_filenames) {
...
Usage: $tool_name [OPTIONS] TRACEFILE(S)
...
sub gen_html()
{
    ...
    print_overall_rate(1, $lcovutil::func_coverage, $lcovutil::br_coverage,
                       $genhtml->top());
}
...
sub write_htaccess_file()
{
    ...
    open(*HTACCESS_HANDLE, ">", $f) or
        die("ERROR: cannot open $f for writing: $!\n");
...
sub write_file_table_entry(*$$@)
{
    ...
    if ($main::flat &&
        ...
        $esc_name = File::Spec->catfile(escape_html($relDir), $esc_name)
            if '.' ne $relDir;
    }
    ...
    if ($is_secondary &&
        ...
        if ($fileview == 0 &&
            ...
            $namecode .=
                "\" title=\"go to coverage summary for $help\">$esc_name</a>";
        }
    }
    ...
    if ($type == $HDR_DIR) {
        ...
        if ($lcovutil::func_coverage) {
            ...
            if ($type == $HDR_SOURCE) {
                if ($sort) {
                    $view .=
                        " (source / <a href=\"$base_name.func-c.$html_ext\">functions</a>)";
                } else {
                    $view .=
    ...
    if ($main::show_tla) {
        my @tlaRow =
            buildHeaderSummaryTableRow($summary, 'line', $fileDetail,
                                  \&SourceFile::nextTlaGroup, $tlaSummaryTypes);
        push(@dataRow, @tlaRow);
    }
...
sub write_file_table(*$$$$$)
{
    ...
    my $file_col_title =
        $fileview ? ('' eq $bin_type ? 'Filename' : 'File') :
        ($main::flat ? 'Filename' : 'Directory');
    my $file_code =
    ...
    if ($main::show_tla) {
        my @visit = (['line', \@lineCovCols]);
        push(@visit, ['branch', \@branchCovCols])
            if $lcovutil::br_coverage;
        push(@visit, ['function', \@functionCovCols])
            if $lcovutil::func_coverage;
        foreach my $d (@visit) {
            my ($covtype, $cols) = @$d;
            foreach my $tla (@{$activeTlaCols->{$covtype}}) {
...
sub write_function_table(*$$$$$$$$$$)
{
    ...
    my $func_code   = funcview_get_label($name, $base, 'function', $type);
