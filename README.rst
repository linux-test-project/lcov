...
``html`` formats.
  ...
  - **man**:
       $ export MANPATH=$MANPATH:$LCOV_HOME/share/lcov/man
       $ man lcov # or 'man genhtml', 'man llvm2lcov', etc.

  - **html**
       point your browser to ``$LCOV_HOME/share/lcov/html/index.html``
...
b) Error handling
   ...
   If environment variable ``LCOV_SHOW_LOCATION`` is set to a non-zero
   integer, then the error manager will emit more detailed debugging
   information into the error log. Higher values generate more
   information.
   More detail is useful to developers in case of a bug in lcov - but is
   unlikely to helpful to regular users.
...
d) Data manipulation
   ...
   of code and/or particular expressions, as well as to do regexp
   replacement of file names - possibly using case-insensitive comparison.
...
e) Callbacks/customization
   ...
   iv) find source files in more complicated environments - where
       simple substitutions become complicated or unwieldy.
   ...
   ix) mark particular branch expressions and/or MC/DC conditions as
       'unreachable' - so they are excluded from the coverage DB.

       Sample script: ``unreach.pm``
...
sensitive or proprietary - then your detailed description is even more
important.
...
Please check back and verify the fix and close the issue once it has
been addressed.
