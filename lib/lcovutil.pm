...
sub write_info_file($$$)
{
  ...
  if ('-' eq $filename) {
    return $self->write_info(*STDOUT, $do_checksum);
  }
  ...
  if ($filename =~ /\.gz$/)
  {
    ...
    # Open compressed file
    open(INFO_HANDLE, "|-", "gzip -c >$filename")
      or die("ERROR: cannot start gzip to compress to file $filename!\n");
  } else {
    open(INFO_HANDLE, ">", $filename)
      or die("ERROR: cannot write to $filename!\n");
  }
  my @result = $self->write_info(*INFO_HANDLE, $do_checksum);
  close(*INFO_HANDLE);
  return @result;
}
